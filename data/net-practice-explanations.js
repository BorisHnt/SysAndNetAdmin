window.NET_PRACTICE_EXPLANATIONS = {
  "01": {
    ...
    calculations: [
      {
        ...
        body: [
          ...
          "Si B vaut 104.97.23.12/24, son réseau est 104.97.23.0 et son broadcast 104.97.23.255. A doit donc commencer par 104.97.23."
        ],
        code: "B : 104.97.23.12/24\nRéseau : 104.97.23.0\nPlage : 104.97.23.1 -> 104.97.23.254\nA possible : 104.97.23.42/24"
      },
      {
        title: "Résoudre le lien C-D en /16",
        body: [
          "Un /16 vaut 255.255.0.0. Seuls les deux premiers octets sont imposés par le réseau.",
          "Si C vaut 211.191.37.75/16, D peut changer ses deux derniers octets tant que l’adresse reste dans 211.191.0.1 à 211.191.255.254."
        ],
        code: "C : 211.191.37.75/16\nRéseau : 211.191.0.0\nBroadcast : 211.191.255.255\nD possible : 211.191.80.42/16"
      },
      {
    ...
    packetTrace: "A 104.97.23.42/24\n  -> destination B 104.97.23.12 appartient à 104.97.23.0/24\n  -> envoi direct\nB reçoit puis répond directement à A",
    verification: [
  ...
  "02": {
    ...
    packet: [
      "Sur A-B, les deux interfaces calculent le réseau 192.168.84.192/27 et envoient directement.",
  ...
  "03": {
    ...
    calculations: [
      ...
      },
      {
        ...
        code: "Réseau commun : 104.198.73.0/25\nIP interdites : .0 et .127\nIP déjà prise : .125"
      }
    ],
    packet: [
  ...
  "04": {
    ...
    intro: [
      ...
      "L’adresse fixe .132 de A et son /28 fournissent un bloc compact qui s’insère entre les autres réseaux."
    ],
    observations: [
      "A, B et R1 sont reliés au même switch.",
      "A possède l’adresse X.Y.Z.132.",
    ...
    calculations: [
      {
        title: "Trouver le bloc de .132 en /28",
        body: [
          "Un /28 laisse 4 bits hôte : 2^4 = 16 adresses. Le pas est 16.",
          "132 tombe entre 128 et 143. Le réseau vaut .128, le broadcast .143 et les hôtes .129-.142."
        ],
        code: "128 <= 132 <= 143\nX.Y.Z.128/28\nHôtes : X.Y.Z.129 -> X.Y.Z.142"
      },
      {
        ...
        code: "R1 LAN : X.Y.Z.129/28\nA : X.Y.Z.132/28"
      },
      {
        title: "Configurer B et éliminer le chevauchement",
        body: [
          "B reçoit une autre adresse libre, par exemple .130, avec le même /28.",
          "Un /23 couvrirait X.Y.(Z pair).0 à X.Y.(Z impair).255 et entrerait en conflit avec les autres interfaces du routeur. Le /28 évite cette ambiguïté."
        ],
        code: "B : X.Y.Z.130/28\nRéseau local unique : X.Y.Z.128/28"
      }
    ],
    packet: [
      "A, B et R1 calculent tous le réseau X.Y.Z.128/28.",
    ...
    verification: [
      "A, B et R1 utilisent-ils exactement /28 ?",
      "Les IP sont-elles dans .129-.142 ?",
    ...
    calculations: [
      {
        ...
        body: [
          "Calcule le réseau de R1 avec son masque. Si R1 vaut 42.50.60.126/25, le réseau est 42.50.60.0/25.",
          "A doit recevoir une IP de .1 à .125, car .126 est déjà utilisée par R1 et .127 est le broadcast."
        ],
        code: "R1 : 42.50.60.126/25\nA : 42.50.60.10/25\nGateway locale A : 42.50.60.126"
      },
      {
        title: "Construire le LAN de B autour de R2",
        body: [
          "Un /18 possède un pas de 64 dans le troisième octet. Si R2 vaut 150.70.130.254/18, 130 tombe dans le bloc 128-191.",
          "B doit donc utiliser 150.70.x.y avec un troisième octet entre 128 et 191 et une adresse différente de R2."
        ],
        code: "R2 : 150.70.130.254/18\nRéseau : 150.70.128.0/18\nB possible : 150.70.130.10/18"
      },
      {
        ...
        code: "A : default via 42.50.60.126\nB : default via 150.70.130.254"
      }
    ],
    packet: [
  ...
  "07": {
    ...
    intro: [
      ...
      "Le moyen le plus simple consiste à utiliser des /30, car chaque segment n’a besoin que de deux adresses."
    ],
    observations: [
    ...
    calculations: [
      {
        title: "Segment A-R1",
        body: [
          "L’adresse X.198.14.1 appartient au bloc X.198.14.0/30.",
          "Les deux hôtes utilisables sont .1 et .2. R1 occupe .1, donc A prend .2."
        ],
        code: "Réseau : X.198.14.0/30\nR1 : X.198.14.1\nA : X.198.14.2"
      },
      {
        title: "Segment R1-R2",
        body: [
          "L’adresse .254 appartient au bloc .252-.255 en /30.",
          "Les adresses utilisables sont .253 et .254. R1 occupe .254, R2 prend .253."
        ],
        code: "Réseau : X.198.14.252/30\nR2 : X.198.14.253\nR1 : X.198.14.254"
      },
      {
        title: "Segment R2-C",
        body: [
          "Il faut un troisième bloc qui ne chevauche pas les deux précédents.",
          "Par exemple X.198.15.0/30, avec R2 en .1 et C en .2."
        ],
        code: "Réseau : X.198.15.0/30\nR2 : X.198.15.1\nC : X.198.15.2"
      },
      {
        ...
        code: "A default via X.198.14.1\nR1 default via X.198.14.253\nR2 default via X.198.14.254\nC default via X.198.15.1"
      }
    ],
    packet: [
      "A remet le paquet à R1, car C n’est pas dans son /30.",
    ...
    packetTrace: "A -> R1 (.1)\nR1 -> R2 (.253)\nR2 -> C sur X.198.15.0/30\nC -> R2 (.1)\nR2 -> R1 (.254)\nR1 -> A",
    verification: [
      "As-tu créé exactement trois sous-réseaux ?",
      "Chaque paire reliée partage-t-elle le même /30 ?",
    ...
    calculations: [
      {
        ...
        body: [
          "Utilise la famille E.198.F.x révélée par les valeurs générées.",
          "Un /25 permet de placer A, B et R1 dans E.198.F.0/25, avec R1 comme gateway locale."
        ],
        code: "R1 LAN : E.198.F.1/25\nA : E.198.F.2/25\nB : E.198.F.42/25\nA/B default via E.198.F.1"
      },
      {
        title: "Construire le transit R1-R2",
        body: [
          "Avec R1 en J.K.17.254, le bloc /30 naturel est J.K.17.252/30.",
          "R2 utilise l’autre adresse disponible, J.K.17.253."
        ],
        code: "J.K.17.252 réseau\nJ.K.17.253 R2\nJ.K.17.254 R1\nJ.K.17.255 broadcast"
      },
      {
        title: "Construire les LAN C et D",
        body: [
          "C peut utiliser J.K.18.0/24 avec R2 comme gateway.",
          "Pour D, ne force pas J.K.19.x si sa route fixe annonce une autre gateway. Si la gateway imposée vaut G1.G2.G3.G4, attribue cette valeur à R2 et choisis D dans le même bloc."
        ],
        code: "LAN C : J.K.18.0/24, R2=.254, C=.1\nLAN D : calculé autour de G1.G2.G3.G4\nExemple /24 : R2=G1.G2.G3.G4, D=G1.G2.G3.10"
      },
      {
        ...
        code: "R1 : réseau C via J.K.17.253\nR1 : réseau D via J.K.17.253\nR1 : default via 163.172.250.1"
      },
      {
  ...
  "10": {
    ...
    intro: [
      "Le dernier niveau reprend la même architecture générale que le niveau 9, mais les adresses fixes suggèrent un plan plus élégant : tous les réseaux internes peuvent être rangés dans A.B.C.0/24.",
    ...
    observations: [
      ...
      "H4 impose le bloc A.B.C.128/26 grâce à son IP .131 et sa gateway fixe .129.",
    ...
    calculations: [
      {
        ...
        body: [
          "A.B.C.0/25 couvre .0-.127. R1 occupe .1 et H1 possède déjà .2.",
          "H2 peut prendre n’importe quelle autre adresse de .3 à .126 avec /25 et utiliser .1 comme gateway."
        ],
        code: "LAN principal : A.B.C.0/25\nR1=.1, H1=.2, H2=.3\nBroadcast=.127"
      },
      {
        title: "Réserver le LAN H4 en /26",
        body: [
          "A.B.C.128/26 couvre .128-.191.",
          "La gateway fixe .129 appartient à ce bloc et doit être attribuée à l’interface R2. H4=.131 est déjà valide."
        ],
        code: "LAN H4 : A.B.C.128/26\nR2=.129\nH4=.131\nBroadcast=.191"
      },
      {
        title: "Réserver le LAN H3 en /27",
        body: [
          "A.B.C.192/27 couvre .192-.223 et ne chevauche pas les deux réseaux précédents.",
          "Attribue par exemple .193 à R2 et .194 à H3. H3 utilise .193 comme gateway."
        ],
        code: "LAN H3 : A.B.C.192/27\nR2=.193\nH3=.194\nBroadcast=.223"
      },
      {
        title: "Placer le transit dans le dernier /30",
        body: [
          "A.B.C.252/30 couvre .252-.255. Les deux adresses utilisables sont .253 et .254.",
          "Elles correspondent précisément aux valeurs fixes de R2 et R1."
        ],
        code: "Transit : A.B.C.252/30\nR2=.253\nR1=.254"
      },
      {
        title: "Écrire les routes hiérarchiques",
        body: [
          "R1 possède déjà une route vers A.B.C.128/26 via R2. Ajoute une route vers A.B.C.192/27 via le même next hop.",
          "R2 utilise R1 comme route par défaut. Internet peut annoncer une seule route A.B.C.0/24 vers l’interface publique de R1."
        ],
        code: "R1 -> A.B.C.128/26 via A.B.C.253\nR1 -> A.B.C.192/27 via A.B.C.253\nR2 -> default via A.B.C.254\nInternet -> A.B.C.0/24 via 163.172.250.12"
      }
    ],
    packet: [
      ...
      "Internet répond avec l’agrégat A.B.C.0/24. R1 reçoit le paquet puis choisit la route locale ou spécifique la plus adaptée."
    ],
    packetTrace: "H2 -> R1 -> route A.B.C.128/26 -> R2 -> H4\nH3 -> R2 -> default R1 -> Internet\nInternet -> A.B.C.0/24 -> R1\nR1 -> /25 local, /26 via R2 ou /27 via R2",
    verification: [
      "Les quatre sous-réseaux occupent-ils des intervalles distincts ?",
      "Tous sont-ils inclus dans A.B.C.0/24 ?",
